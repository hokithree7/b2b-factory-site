...
for url in urls:
    ...
    try:
        ...
        # Ensure the file input itself has name=\"attachment\"
        # Replace any input type=\"file\" that doesn't have a name
        html = re.sub(r'<input\s+([^>]*)type=\"file\"(?! [^>]*name=)', r'<input \1 type=\"file\" name=\"attachment\"', html)
        # If it already has an empty name, fix it
        html = html.replace('name=\"\" type=\"file\"', 'name=\"attachment\" type=\"file\"')
        html = html.replace('type=\"file\" name=\"\"', 'type=\"file\" name=\"attachment\"')

        # 3. YouTube Link Fix (Avoid double https://)
        # First remove all protocols, then add the correct one
        ...
        html = html.replace('\"https://www.youtube.com/@HLKTape\"', '\"https://www.youtube.com/@HLKTape\"') # Noop for sanity
