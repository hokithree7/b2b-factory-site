...
$(document).ready(function () {
  ...
  var $INQURY_BOX = $(".crm_inquiry_form_boxcustom_inquiry_form_1744007317884380");
...
$(function () {
  ...
  var swiper = new Swiper('#arshine2-banner .swiper', {
    ...
    pagination: {
      ...
      el: '#arshine2-banner .swiper-pagination',
...
jQuery(function($) {
  ...
  $("#do-slider").owlCarousel({
    ...
    navContainer: '#building-owl-nav',
    navText: ["<i class='qico qico-left'></i>","<i class='qico qico-right'></i>"],
    dots: false,
    nav: true,
    touchDrag: true,
...
(function($) {
  ...
  $('.atf__service-slider-active').slick({
    ...
    slidesToScroll: 1,
    rtl: $("html").attr("dir") === 'rtl',
    prevArrow: '<a class="slick-prev" aria-label="go prev"><i class="qico qico-left2" alt="Arrow Icon"></i></a>',
    nextArrow: '<a class="slick-next" aria-label="go next"><i class="qico qico-right2" alt="Arrow Icon"></i></a>',
    responsive: [
      {
        breakpoint: 1200,
        settings: {
...
$(function () {
  ...
  $(document).on('click', function(e) {
    var $target = $(e.target);
    console.log($target)
    if ($target.closest('.form-input').length) {
      // 执行你想在点击div以外区域时进行的操作
      // 例如：隐藏div
      $(".parquetry-faq-block .form-label").addClass("focus");
    }else{
      if($(".parquetry-faq-block #subscribe-form-5-email").val()===""){
        $(".parquetry-faq-block label[for='subscribe-form-5-email']").removeClass("focus");
      }
    }
  });
...
$(document).ready(function () {
  ...
  var $INQURY_BOX = $(".crm_inquiry_form_boxcustom_inquiry_form_1750295603869365");
  var crmEmailStr = "Please enter a valid email address";
  var pagetitle = '';
  $('#pagetitle').val(document.title);
